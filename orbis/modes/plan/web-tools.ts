...
function getClient(apiKey?: string): Firecrawl {
...
export function createWebTools(tracker: ActionTracker, apiKey?: string) {
    ...
    web_search: tool({
      ...
      execute: async ({ query, limit }) => {
        try {
          ...
          const msg = `web_search failed for "${query}": ${errMsg(err)}`;
          tracker.log({
            type: "code_analysis",
            path: `web_search:${query}`,
            details: { after: msg, toolName: "web_search" },
            status: "executed",
          });
          return `(${msg})`;
        }
    ...
    web_crawl: tool({
      ...
      execute: async ({ url }) => {
        try {
          ...
          const msg = `web_crawl failed for ${url}: ${errMsg(err)}`;
          tracker.log({
            type: 'code_analysis',
            path: `web_crawl:${url}`,
            details: { after: msg, toolName: 'web_crawl' },
            status: 'executed',
          });
          return `(${msg})`;
        }
    ...
    fetch_url: tool({
      ...
      execute: async ({ url }) => {
        try {
          ...
          const msg = `fetch_url failed for ${url}: ${errMsg(err)}`;
          tracker.log({
            type: 'code_analysis',
            path: `fetch:${url}`,
            details: { after: msg, toolName: 'fetch_url' },
            status: 'executed',
          });
          return `(${msg})`;
        }
